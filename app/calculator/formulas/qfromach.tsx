...
import { ArrowLeft, Upload, Wind, Info, Loader2, Sun, Moon, CheckCircle2, X, RefreshCw } from "lucide-react";
...
export default function Qfromach() {
    ...
    const fetchDayRows = useCallback(async (year: number, month: number, day: number) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setDayError(data.message || "No data returned from API.");
            }
        } catch (e) {
    ...
    // ── Row select → auto-fill ΔT ─────────────────────────────────────────────
    const handleRowSelect = (row: EPWHourRow) => {
        setSelectedHour(row.hour);
        setSelectedDate(prev => ({ ...prev, hour: row.hour }));
        const t_o = row.temperature;
        const newDeltaT = parseFloat(Math.abs(values.t_i - t_o).toFixed(2));
        setValues(prev => ({ ...prev, delta_T: newDeltaT }));
        setEpwMessage(
            `Selected ${formatHour(row.hour)}: Outdoor ${t_o}°C → ΔT = ${newDeltaT} K`
        );
    };
    ...
    // ── Mobile: Apply button (fallback for when viewer is hidden) ─────────────
    ...
    const tabActiveBg = isDark ? "#1A73E8" : "#1A73E8";
    const tabInactiveBg = isDark ? "rgba(255,255,255,0.05)" : "#f1f5f9";
                        ...
                        <div className="space-y-4">
                            ...
                            {uploadState === "uploading" && (
                                <div
                                    ...
                                    style={{ background: selectBg, border: cardBorder }}
                                >
                            ...
                            {uploadState === "uploaded" && (
                                <div
                                    className="flex items-center gap-3 px-4 py-3 rounded-xl animate-in fade-in duration-300"
                                    style={{ background: selectBg, border: "1px solid rgba(34,197,94,0.3)" }}
                                >
                                    ...
                                    <button
                                        onClick={handleReplaceEpw}
                                        title="Replace file"
                                        className="flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg transition-colors hover:bg-[#1A73E8]/10"
                                        style={{ color: "#1A73E8", border: "1px solid rgba(26,115,232,0.3)", whiteSpace: "nowrap" }}
                                    >
                                        <RefreshCw className="h-3 w-3" />
                                        Replace
                                    </button>
                                    <button
                                        onClick={handleRemoveEpw}
                                        title="Remove file"
                                        className="flex items-center justify-center h-7 w-7 rounded-lg transition-colors hover:bg-red-500/10"
                            ...
                            {uploadState === "error" && (
                                <div
                                    ...
                                    style={{ background: selectBg, border: "1px solid rgba(239,68,68,0.4)" }}
                                >
                            ...
                            {uploadState === "uploaded" && (
                                <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
                                    {/* Year / Month / Day selectors (always visible) */}
                                        ...
                                        {dayLoading ? (
                                            ...
                                            <div className="rounded-xl overflow-hidden" style={{ border: cardBorder }}>
                                                ...
                                                <div className="flex" style={{ background: tableHeaderBg, borderBottom: cardBorder }}>
                                                    <button
                                                        onClick={() => setViewerTab("AM")}
                                                        className="flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-semibold uppercase tracking-wider transition-all duration-200"
                                                        style={{
                                                            background: viewerTab === "AM" ? tabActiveBg : "transparent",
                                                            color: viewerTab === "AM" ? "#ffffff" : tabInactiveColor,
                                                            borderRadius: "10px 10px 0 0",
                                                        }}
                                                    >
                                                        <Sun className="h-3.5 w-3.5" />
                                                        AM &nbsp;<span className="opacity-60 font-normal normal-case tracking-normal">1 – 12</span>
                                                    </button>
                                                    <button
                                                        onClick={() => setViewerTab("PM")}
                                                        className="flex-1 flex items-center justify-center gap-2 py-2.5 text-xs font-semibold uppercase tracking-wider transition-all duration-200"
                                                        style={{
                                                            background: viewerTab === "PM" ? tabActiveBg : "transparent",
                                                            color: viewerTab === "PM" ? "#ffffff" : tabInactiveColor,
                                                            borderRadius: "10px 10px 0 0",
                                                        }}
                                                    >
                                                        <Moon className="h-3.5 w-3.5" />
                                                        PM &nbsp;<span className="opacity-60 font-normal normal-case tracking-normal">13 – 24</span>
                                                    </button>
                                                </div>
                                                ...
                                                {activeRows.map((row) => {
                                                    ...
                                                    return (
                                                        <div
                                                            ...
                                                            className="grid px-4 py-2.5 cursor-pointer transition-all duration-150 group"
                                                            style={{
                                                                gridTemplateColumns: "1fr 1fr 1fr 1fr",
                                                        ...
                                                        >
                                                            <span
                                                                ...
                                                                style={{ color: isSelected ? "#1A73E8" : titleColor }}
                                                            >
                                                                {formatHour(row.hour)}
                                                            </span>
                                                            <span className="text-sm text-right font-mono" style={{ color: isSelected ? "#1A73E8" : subtitleColor }}>
                                                                {row.temperature.toFixed(1)}
                                                            </span>
                                                            <span className="text-sm text-right font-mono" style={{ color: isSelected ? "#1A73E8" : subtitleColor }}>
                                                                {row.wind_speed_ms.toFixed(2)}
                                                            </span>
                                                            <span className="text-sm text-right font-mono" style={{ color: isSelected ? "#1A73E8" : subtitleColor }}>
                                                                {row.wind_speed_mh.toFixed(0)}
                                                            </span>
                                                        </div>
                                                    );
                                                })}
                                                ...
                                                {activeRows.length === 0 && (
                                                    <div className="py-6 text-center text-sm" style={{ color: labelColor }}>
                                                        No {viewerTab} data available for this day.
                                                    </div>
                                                )}
                                            </div>
                                        ) : null}
                                    ...
                                    {/* Status message */}
                                    {epwMessage && uploadState !== "uploaded" && (
